@extends('layouts.app')
@section('title', 'Chat Général')
@section('page-title', 'Chat Général 💬')
...
<div class="flex gap-4 h-[calc(100vh-12rem)]">
    ...
    <div class="w-64 bg-white dark:bg-gray-800 rounded-2xl border-2 border-orange-200 dark:border-orange-800 shadow-sm flex flex-col">
        <div class="px-4 py-3 border-b border-orange-100 dark:border-orange-900">
            <h3 class="font-semibold text-gray-800 dark:text-white text-sm">👥 Utilisateurs</h3>
        </div>
        <div class="flex-1 overflow-y-auto p-3 space-y-2">
            @foreach($users as $u)
            <a href="{{ route('chat.private', $u) }}"
               class="flex items-center gap-3 p-2 rounded-xl hover:bg-orange-50 dark:hover:bg-orange-900/30 transition">
    ...
    <div class="flex-1 bg-white dark:bg-gray-800 rounded-2xl border-2 border-orange-200 dark:border-orange-800 shadow-sm flex flex-col">
        ...
        <div class="px-6 py-4 border-b border-orange-100 dark:border-orange-900 flex items-center gap-3">
            <div class="w-10 h-10 bg-orange-100 dark:bg-orange-900 rounded-xl flex items-center justify-center text-xl">
                💬
            </div>
            <div>
                <h3 class="font-semibold text-gray-800 dark:text-white">Chat Général</h3>
                <p class="text-xs text-gray-400">Tous les membres</p>
            </div>
        </div>
        ...
        <div id="messages-container" class="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <div class="flex {{ $msg->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }} gap-2">
                @if($msg->sender_id !== auth()->id())
                <div class="w-7 h-7 rounded-full bg-orange-100 dark:bg-orange-900 flex items-center justify-center text-orange-600 text-xs font-bold flex-shrink-0">
...
<script>
    ...
    const sendUrl = "{{ route('chat.group.send') }}";
    ...
    function sendMessage() {
        ...
        .then(res => res.json())
        .then(data => {
            appendMessage({ ...data, sender: { name: 'Vous' } }, true);
            input.value = '';
        });
    }
    ...
    function appendMessage(data, isMine) {
        ...
        div.innerHTML = `
            ${!isMine ? `<div class="w-7 h-7 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 text-xs font-bold flex-shrink-0">${data.sender.name.substring(0,2).toUpperCase()}</div>` : ''}
            <div class="max-w-xs lg:max-w-md">
                ${!isMine ? `<p class="text-xs text-gray-400 mb-1 ml-1">${data.sender.name}</p>` : ''}
